class Wonga::Pantry::Ec2InstancesUpdater
  ...
  def update_from_aws
    ...
    @ec2.instances.each do |aws_instance|
      ...
      elsif with_aws_tags?(aws_instance)
      ...
      Rails.logger.info("Updated records from AWS: Errors #{results['errors'].count},"\
                        "Unchanged #{results['unchanged'].count}, Untagged #{results['untagged'].count}, Updated #{results['updated'].count}")
    end
    results
  end
  ...
  def with_aws_tags?(aws_instance)
    cf_tags = [
    ...
    aws_instance.tags.to_h.any? { |k, _v| cf_tags.include?(k) }
  end
end
