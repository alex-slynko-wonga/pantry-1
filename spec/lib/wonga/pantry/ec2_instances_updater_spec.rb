...
RSpec.describe Wonga::Pantry::Ec2InstancesUpdater do
  ...
  describe '#update_from_aws' do
    ...
    context 'with valid token' do
      ...
      before(:each) do
        ...
        describe_tags_hash[:tag_set] = aws_tags.map do |tag_key, tag_value|
          {
        ...
        describe_instances_hash[:instance_index] = {
          aws_instance.instance_id => {
            tag_set: aws_instance.tags.map do |tag_key, tag_value|
              {
                key: tag_key,
                value: tag_value
              }
